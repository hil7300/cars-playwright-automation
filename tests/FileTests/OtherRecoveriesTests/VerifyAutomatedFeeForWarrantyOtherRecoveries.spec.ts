...
const WARRANTY_DATA = {
    ...
    warrantyAmount: '1500', // Set high enough to cover all test scenarios
...
test.describe('Verify Warranty Status Updates on Other Recoveries', () => {
    ...
    for (const data of TIER_TEST_CASES) {
        test(`Verify sequential Warranty Status Updates - ${data.tier}`, async ({ page, loginAs }) => {
            ...
            await test.step('Verify Company Fee Schedule configuration', async () => {
                ...
                await expect(page.locator(companiesPage.fee_schedule_row_selector).filter({ hasText: 'Warranty Fee' })).toBeVisible();
                await expect(page.locator(companiesPage.fee_schedule_row_selector).filter({ hasText: 'Insurance Fee' })).toBeVisible();
            });
            ...
            await test.step(`Action: Set Initial Status to ${WARRANTY_DATA.initialStatus}`, async () => {
                ...
                await page.getByRole('option', { name: WARRANTY_DATA.initialStatus, exact: true }).click();

                await otherRecoveries.company_dropdown.click();
                await page.getByRole('option', { name: WARRANTY_DATA.companyName, exact: true }).click();
            ...
            await test.step(`Progress Warranty Status to Refund Received (${data.formattedRefundAmount})`, async () => {
                await otherRecoveries.applyStatusUpdate('Warranty Rebate Required', 'Claim Submitted');
            ...
            await test.step(`Verify Warranty Fee is calculated correctly for ${data.tier}`, async () => {
                const feeRow = page.locator(otherRecoveries.fee_row_selector).filter({ hasText: 'Warranty Fee' });
                ...
                await invoiceTextbox.fill(WARRANTY_DATA.invoiceNumber);
                await saveButton.click();

                // Verify the edit was successful
                await expect(feeRow).toContainText(WARRANTY_DATA.invoiceNumber);
            });
        });
    }
});